...
// * Fully encapsulated pin variables, allowing multiple L298N 
//   instances (e.g., 4-wheel drive robots) without pin conflicts.
//
// CONSTRUCTOR:
...
//       Send an order to motors. If no direction given, go straight ahead.
...
// (**)  Masters work at speed provided, slave at percent (lower) speed.
// (***) inv = inverted, ENA/ENB = PWM speed if 1
//
// TROUBLESHOOTING:
// * Left is right, right is left! - Exchange motor wires A<->B (hardware method)
//   or put invert _true_ when creating the object (software method).
//   E.g.: L298N driver(ENA, IN1, IN2, IN3, IN4, ENB, true, MINSPEED);
// * Motors do not work! - Remember to use PWM pins for ENA/ENB: 3, 5-6, 9-11
...
//   reflect bits used to configure motors, no need to code it separately. You
...
class L298N
{
  public:
    ...
    void smoothDrive(uint8_t direction = FORWARD, uint8_t start_speed = 0, uint8_t end_speed = 255, int step_delay = 5);
